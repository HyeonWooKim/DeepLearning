...
from space_player import SpacePlayer
import tensorflow as tf
import numpy as np
import cv2
from pygame.constants import K_DOWN, K_UP, K_LEFT, K_RIGHT, K_f


class DeepSpacePlayer(SpacePlayer):
    ACTIONS_COUNT = 6  # number of valid actions. In this case up, still and down
    ...
    def __init__(self, checkpoint_path="logs", playback_mode=True, verbose_logging=False):
        """
        ...
        super(DeepSpacePlayer, self).__init__(force_game_fps=128, run_real_time=playback_mode)
        self.verbose_logging = verbose_logging
        self._checkpoint_path = checkpoint_path
        self._session = tf.Session()
        self._input_layer, self._output_layer = DeepSpacePlayer._create_network()
    ...
    def get_keys_pressed(self, screen_array, reward, terminal):
        ...
        if self._last_state is None:
            ...
            return DeepSpacePlayer._key_presses_from_action(self._last_action)
    ...
    def _choose_next_action(self):
        ...
        if (not self._playback_mode) and (random.random() <= self._probability_of_random_action):
           ...
           # print("Action Q-Values are %s" % readout_t)
            action_msg = ["down","stay","up","left","right","shoot"]
            action_index = np.argmax(readout_t)
           # print "DeepLearning Action : ",action_msg[action_index]
    ...
    def _create_network():
        # network weights
        convolution_weights_1 = tf.Variable(tf.truncated_normal([8, 8, DeepSpacePlayer.STATE_FRAMES, 32], stddev=0.01))
        ...
        feed_forward_weights_2 = tf.Variable(tf.truncated_normal([256, DeepSpacePlayer.ACTIONS_COUNT], stddev=0.01))
        feed_forward_bias_2 = tf.Variable(tf.constant(0.01, shape=[DeepSpacePlayer.ACTIONS_COUNT]))

        input_layer = tf.placeholder("float", [None, DeepSpacePlayer.RESIZED_SCREEN_X, DeepSpacePlayer.RESIZED_SCREEN_Y,
                                               DeepSpacePlayer.STATE_FRAMES])
